...
USBD_CDC_LineCodingTypeDef LineCode = {115200, 0x00, 0x00, 0x08};
...
static int8_t CDC_Control_FS(uint8_t cmd, uint8_t *pbuf, uint16_t length) {
  ...
  case CDC_SET_LINE_CODING:
    ...
    LineCode.bitrate = (uint32_t)(pbuf[0] | (pbuf[1] << 8) | (pbuf[2] << 16) |
                                  (pbuf[3] << 24));
...
static int8_t CDC_Receive_FS(uint8_t *Buf, uint32_t *Len) {
  /* USER CODE BEGIN 6 */
  /*
   * NOTE: No backpressure — if the ring buffer is full, data is dropped.
   * USB re-arms unconditionally. This is acceptable at current throughput
   * but should be revisited if message rates increase significantly.
   */
