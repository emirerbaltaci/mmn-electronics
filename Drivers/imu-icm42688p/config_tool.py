...
class ConfigTool:
    def __init__(self, root):
        self.root = root
        self.root.title("IMU Configuration Tool")
    ...
    def parse_file(self):
        ...
        # Regex patterns
        re_config_header = re.compile(r"^\s*\*\s+(IMU_SETUP_[A-Za-z0-9_]+)\s+(.+)$")
        re_option = re.compile(r"^\s*\*\s+(.+?)\s{2,}(.+)$") 
        re_define = re.compile(r"^#define\s+(IMU_SETUP_\w+)\s+(.+)$")
        ...
        for idx, line in enumerate(self.file_lines):
            ...
            if in_comment_block:
                ...
                # Check formatting
                # Header: * IMU_SETUP_...   Description
                parts = re.split(r'\s{2,}', content)
                
                if len(parts) >= 2 and parts[0].startswith("IMU_SETUP_"):
                    ctx_desc = parts[1]
                ...
                if match_def:
                    ...
                    if current_context:
                        ...
                        # Note: INTx pattern. The context might be named INTx, but define is INT1. 
                        # We just apply the context options to this define.
    ...
    def save_config(self):
        ...
        for cfg in self.configs:
            ...
            # Skip update if value hasn't changed to avoid touching file unnecessarily? 
